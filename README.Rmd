...
+ [`get_glassbrain()`](#id16) Generates a 3D plot of cells counts with option of removing glassbrain in the background.
...
setup <- interpolate(setup, method = "s" )
```
...
\newpage
<a id="id16"></a> 
**`get_glassbrain()` ** A modified version of wholebrain::glassbrain(). New options include:
...
glassbrain <- get_glassbrain(dataset, high.res = "OFF", jitter = FALSE) 
...
> Tip: Combine the `get_rois()` function output with `get_glassbrain()` to get a glass brain of just ROIs.
