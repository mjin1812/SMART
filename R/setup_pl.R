...
#' Generates a setup list of length 7 if the user is registering a partial brain or of length 16 if the user is mapping a whole brain.
#' If a setup list already exists, calling this function will simply ask the user to verify setup info.
#' Info can be changed if necessary. However, be conservative with changing information, especially if different users work on the same brain.
#' @param setup (optional) Allows the user to modify a setup list previously generated by this function.
#' @details
...
#' 7)  Spacing between registrations (mm). Default is 0.100, meaning registrations are performed
#'     every 100 um. Recommended input is are multiples of 0.100.
#'     Beware! Larger spacing between registrations means less manual work but less accurate mapping
#'     in the rostral caudal axis.
#' 8)  Step size between z-images used for segmentation. (integer). Default is 1 (1 skips no
#'     images, 2 skips every other, etc.).
#' 9)  First AP coordinate
#' 10) First z image. Note: Keep all images of the stack together, even if the first usable
#'     image isn't 1.
#' 11) Last AP coordinate
#' 12) Last z image
#' 13) AP coordinates of internal reference planes.
#'     Default coordinates are 1.91, 1.10, -.42, -0.93 , -1.94 , -2.95, -3.96.
#'     They correspond to PFC, NAc, antHyp, startHip, posHyp, VTA, PAG, respectively.
#' 14) z image numbers matching AP coordinates (14). Will be NULL until wb_choice() is run.
#' 15) All AP coordinates ranging from the first to last AP. Will be NULL until wb_interpolate() is run.
#' 16) All z image numbers matching 16. Will be NULL until wb_interpolate() is run.
