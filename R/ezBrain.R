#' ezBrain: This package builds a pipeline interfacing with the [wholebrain]
#' package to process whole brain imaging datasets.
#'
#' @description The pipeline is split into five different parts, with the purpose
#' of each function per part explained below. Note that all functions that are
#' exclusively meant to be used with whole brain datasets will be marked with a
#' **(W)** next to the function. Unmarked functions can be used on single slice
#' datasets across the brain.
#'
...
#' 3) [ez_interpolate()] **(W)** Interpolate all AP and z numbers
#'    for atlas plates in a whole brain project
#'
#' @section Part 3. Registration:
#'
#' 1) [ez_regi()] Console user interface built on top of
